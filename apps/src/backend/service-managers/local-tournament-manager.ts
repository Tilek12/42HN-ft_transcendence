...
class LocalTournamentManager {
    ...
    public async startLocalTournament(id: number) {
        ...
        // Broadcast initial bracket (all first-round matches defined)
        this.broadcastLocalTournamentUpdate(tournament.id);
    ...
    /** Start one match, wire onEnd → TournamentManager.onMatchEnd */
    ...
    async onMatchEnd(
        ...
        // Broadcast updated bracket after this match ends
        this.broadcastLocalTournamentUpdate(tournament.id);
        ...
        if (round && round.every(m => m.status === 'finished')) {
            ...
            if (winners.length === 1) {
                ...
                if (ctrlSocket && ctrlSocket.readyState === WebSocket.OPEN) {
                    ctrlSocket.send(JSON.stringify({
                        ...
                        winner: { id: win!.id, name: win!.name }
                    }));
                }
            ...
            // Broadcast after new round is created (so frontend sees new matches)
            this.broadcastLocalTournamentUpdate(tournament.id);
