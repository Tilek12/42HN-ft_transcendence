...
const friend_request_action = (peding: boolean, other_profile_id: number, blocksMe:boolean) => {
	...
	if (!peding)
		...
		res =/*html*/
			`<button 
				data-profile-id = "${other_profile_id}" 
				class="pending-btn px-4 py-2 bg-gray-600 text-white opacity-50 rounded-lg flex items-center cursor-not-allowed">
...
const block_action = (iBlock: boolean, other_profile_id: number) => {
	...
	if (!iBlock)
		...
		res =/*html*/
			`<button 
			data-profile-id = "${other_profile_id}" 
			class="unblock-btn px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg transition-all duration-300 flex items-center shadow-md hover:shadow-xl transform hover:scale-110 group">
...
// const profile_to_html = (profile: fProfileListEntry, ): string => {
// 	let is_connected;
...
// 						<span id="profiles-loggin-state-${profile.username}" class="absolute bottom-0 right-0 w-5 h-5 rounded-full border-2 border-white shadow-lg transition-all duration-300"></span>
// 					</div>
...
export async function showError(id: errorID, res?: Response, message?: string) {
	...
	if (res) {
		...
		errorBox.innerText = `${res.status} ${data.message} ${message ? message : ''}`
	}
	else {
		errorBox.innerText = `${translations_errors[languageStore.language].error_default} ${message ? message : ''}`;
	}
	// setTimeout(() => { errorBox.classList.add('hidden') }, 2000)
}

// ZERO init the Profiles List
let profilesList: fProfileList = {
	profiles: [],
	offset: 0,
	limit: 3,
