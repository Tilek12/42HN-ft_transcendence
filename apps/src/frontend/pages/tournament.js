...
async function renderTournament(root) {
  ...
  `);
    ...
    ws_manager_1.wsManager.subscribeToPresence(renderTournamentList);
    ...
    function renderTournamentList() {
        ...
        list.innerHTML = '';
        const tournaments = ws_manager_1.wsManager.onlineTournaments;
        const token = (0, auth_1.getToken)();
        const userId = token ? JSON.parse(atob(token.split('.')[1])).id : null;
      ...
      `;
            ...
            infoBox.querySelector('#quit-tournament-btn')?.addEventListener('click', () => {
                ws_manager_1.wsManager.quitTournament();
                currentTournamentId = null;
                alert('🚪 You left the tournament.');
                ws_manager_1.wsManager.disconnectTournamentSocket();
                renderTournamentList();
            });
        }
    ...
    function joinTournament(id, size) {
        if (currentTournamentId) {
            alert(`⚠️ You're already in Tournament ${currentTournamentId}.`);
            return;
        }
        const socket = ws_manager_1.wsManager.connectTournamentSocket('join', size, id, (msg) => {
    ...
    function createTournament(size) {
        ...
        const socket = ws_manager_1.wsManager.connectTournamentSocket('create', size, undefined, (msg) => {
            handleTournamentMessage(msg);
            if (msg.type === 'tournamentJoined') {
                currentTournamentId = msg.id;
